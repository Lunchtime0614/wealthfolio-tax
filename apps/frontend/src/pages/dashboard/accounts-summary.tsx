...
const GroupTaxTreatmentSummary = React.memo(
  ({ accounts, baseCurrency }: { accounts: AccountSummaryDisplayData[]; baseCurrency: string }) => {
    const taxTotals = useMemo(() => {
      const totals: Record<
        string,
        { treatment: string; totalValue: number; accountCount: number }
      > = {};
      ...
      accounts.forEach((account) => {
        ...
        if (!totals[treatment]) {
          totals[treatment] = { treatment, totalValue: 0, accountCount: 0 };
        }
    ...
    if (taxTotals.length <= 1) {
      return null;
    }

    return (
      <div className="space-y-2 px-4 py-3 md:px-5 md:py-4">
        <p className="text-muted-foreground mb-3 text-xs font-semibold">Tax Treatment Breakdown</p>
        <div className="grid gap-2">
          {taxTotals.map((total) => (
            <div
              key={total.treatment}
              className="bg-muted/30 flex items-center justify-between gap-3 rounded p-2"
            >
...
export const AccountsSummary = React.memo(
  ({ dateRange, isAllTime }: { dateRange?: DateRange; isAllTime?: boolean }) => {
    const { groupingMode, setGroupingMode, settings } = useSettingsContext();
